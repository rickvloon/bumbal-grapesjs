...
export default (editor: Editor, opts: Required<PluginOptions>) => {
	editor.onReady(() => {
		...
		header.innerHTML = `
			<div class="gjs-variables-title">
				...
				<span>${t("variables.panelTitle")}</span>
			</div>
			<div class="gjs-variables-actions">
				<button type="button" class="gjs-variables-help" title="${t("variables.help")}">
				...
				<button type="button" class="gjs-variables-toggle" aria-label="${t("variables.toggle")}">
			...
			Object.entries(items).forEach(([name, code]) => {
				...
				itemEl.addEventListener("click", () => {
					navigator.clipboard.writeText(code).then(() => {
						const original = name;
						itemEl.textContent = t("variables.copied");
	...
	editor.on("rte:enable", (view, rte) => {
		...
		wrapper.innerHTML = t("variables.mergeMenuLabel");
