...
export default function (editor: Editor, opts: Required<PluginOptions>) {
	...
	const category = t("blocks.category.layouts");

	addBlock("sect100", {
		label: t("blocks.sect100"),
		category,
...
`,
	...
	const basicCategory = t("blocks.category.basic");
...
`,
	...
	const customCategory = t("blocks.category.custom");

	addBlock("packageline", {
		label: t("blocks.packageline"),
...
`,
		// Wrapped in a div so "Button align" (which centers/aligns the button
		...
		content: `<div style="text-align: center;"><a class="button" style="display: inline-block; font-size: 14px; color: #ffffff; background-color: #006FBF; padding: 10px 20px; border-radius: 4px; text-decoration: none;">${t("content.buttonText")}</a></div>`,
	});
}
